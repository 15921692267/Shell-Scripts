...
#磁盘利用率与I/O负载（df、iostat、dstat）
#内存利用率（free、vmstat）
#TCP连接状况（netstat、ss）
#CPU与内存占用进程TOP10（ps）
#实时流量进出（ifconfig、iftop、iptraf）
#磁盘Inodes利用率（df -i）
        ...
        case $input in
            ...
            traffic)
                ...
                while true; do
                    ...
                    if [ `ifconfig |grep -c "\<$eth\>"` -ne 1 ]; then
                ...
                while [[ $i -le 3 ]]; do
                    ...
                    #带宽速率计算方式是：1M=1024kbps=128KByte/s（1024kbps/8bit），所以1M=128Kb，(1Byte=8bit,大写B是Byte，小写b是bit)
                    #因此1024byte/1024=1KB（等于1024英文字母及字符，或512汉字），1024KB/128Kb=8M。
