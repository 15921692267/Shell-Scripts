...
reload_qemu(){
	service libvirtd restart
        ...
        sleep 1
	/usr/bin/python /usr/share/virt-manager/virt-manager.py
   ...
   add)
	while true; do
		...
		while true; do
			...
			judge=${judge:-test}
			if [ $judge == YES -o $judge == y ]; then
				[ `virsh list --all |awk '$2=="'$vm_name'"{print $3}'` == "running" ] && virsh shutdown $vm_name && sleep 3
				cp -v $xml_template_file $xml_file
				init_xml
		    		if `virsh start $vm_name >/dev/null 2>&1`; then
					echo "$vm_name 虚拟机初始化成功,并且启动."
					exit
					#reload_qemu
				else
